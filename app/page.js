...
export default function Home() {
  ...
  return (
    <main>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/95 backdrop-blur border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative flex items-center justify-between py-2">
            {/* Пустой блок слева — для баланса на мобильном */}
            <div className="w-10 h-10 flex-shrink-0 md:hidden"></div>
            ...
            <button onClick={() => setMenuOpen(true)} className="w-10 h-10 flex items-center justify-center rounded-md text-gray-900 hover:text-blue-600 flex-shrink-0 md:hidden">
          ...
          {/* Нижняя строка — меню по центру */}
          <div className="flex justify-center items-center pb-2 md:pb-3">
            <ul className="flex justify-center items-center gap-6 md:gap-10 list-none">
              <li className="relative">
                <button
                  onClick={() => setGalleryOpen(!galleryOpen)}
                  className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium flex items-center gap-1 whitespace-nowrap"
                >
                ...
                {galleryOpen && (
                  <>
                    <div className="fixed inset-0 z-40" onClick={() => setGalleryOpen(false)}></div>
                    <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 min-w-[180px] z-50">
                      {categories.map((cat) => (
                        <button
                          key={cat}
              ...
              <li>
                <button onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })} className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium whitespace-nowrap">
                  {currentLang.nav.contacte}
                </button>
              </li>
              <li>
                <button onClick={() => document.getElementById('despre')?.scrollIntoView({ behavior: 'smooth' })} className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium whitespace-nowrap">
                  {currentLang.nav.despre}
                </button>
              </li>
            </ul>
          </div>
          ...
          <div className="flex flex-col h-full p-6 pt-20 relative">
            ...
            <div className="flex justify-center gap-4 mt-8">
              <div className="flex items-center bg-gray-100 rounded-full p-1">
                <button onClick={() => changeLanguage('ro')} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${language === 'ro' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}>RO</button>
                <button onClick={() => changeLanguage('en')} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${language === 'en' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 hover:text-gray-900'}`}>EN</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ====== HERO СЛАЙДЕР ====== */}
      <section className="relative h-[70vh] md:h-[80vh] overflow-hidden mt-28 md:mt-32">
        {slides.map((slide, i) => (
