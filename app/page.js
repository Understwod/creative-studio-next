...
export default function Home() {
  ...
  return (
    <main>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 bg-white/95 backdrop-blur border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative flex justify-center items-center py-2">
            ...
            {/* Бургер — всегда */}
            <button onClick={() => setMenuOpen(true)} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 rounded-md text-gray-900 hover:text-blue-600">
          ...
          {/* Нижняя строка — меню всегда видно */}
            ...
            <ul className="flex gap-4 md:gap-8 list-none whitespace-nowrap">
              <li className="relative">
                <button
                  ...
                  className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium flex items-center gap-1"
                >
              ...
              <li><button onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })} className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium">{currentLang.nav.contacte}</button></li>
              <li><button onClick={() => document.getElementById('despre')?.scrollIntoView({ behavior: 'smooth' })} className="text-xs md:text-sm uppercase tracking-wide text-gray-900 hover:text-blue-600 font-medium">{currentLang.nav.despre}</button></li>
            </ul>
          </div>
        </div>
      </nav>
      ...
      {/* Открытое мероприятие — фото по одному в столбик на мобильном */}
      {selectedEvent && (
