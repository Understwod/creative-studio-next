...
export default function Home() {
  ...
  // Остальная логика (слайдер, лайтбокс)
  useEffect(() => {
  ...
  return (
    <main>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 flex justify-between items-center px-6 py-4 bg-white/95 backdrop-blur border-b border-gray-100">
        <a href="#" className="flex items-center"><Image src="/logo.png" alt="Creative Studio" width={150} height={50} className="h-10 w-auto" priority /></a>
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="md:hidden flex flex-col gap-1.5 p-2"><span className="w-6 h-0.5 bg-black"></span><span className="w-6 h-0.5 bg-black"></span><span className="w-6 h-0.5 bg-black"></span></button>
      </nav>
      ...
      {selectedWedding && (
        <div className="fixed inset-0 bg-black/95 z-[9998] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-6">
            {loadingWedding ? <p className="text-white">Se încarcă...</p> : photos.map((photo, i) => (
              <div key={photo.id} className="cursor-pointer" onClick={() => openLightbox(i)}>
                <img src={photo.image_url} alt={photo.caption} className="w-full h-48 object-cover rounded-lg" />
              </div>
            ))}
          </div>
        </div>
      ...
      {lightboxOpen && (
        <div className="fixed inset-0 bg-black/95 z-[9999] flex items-center justify-center" onClick={() => setLightboxOpen(false)}>
          ...
          <img src={photos[currentPhotoIndex].image_url} className="max-w-[90%] max-h-[90%] rounded-lg" onClick={(e) => e.stopPropagation()} />
      ...
      <section id="servicii" className="py-20 px-6 max-w-6xl mx-auto text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
          <div className="bg-gray-50 p-8 rounded-2xl reveal">
            ...
            <ul className="mt-4 text-left text-gray-500 space-y-2"><li>1 fotograf</li><li>150 fotografii</li><li>Videoclip scurt</li></ul>
          ...
          <div className="bg-gray-50 p-8 rounded-2xl reveal border-2 border-blue-500">
            ...
            <ul className="mt-4 text-left text-gray-500 space-y-2"><li>2 fotografi + videograf</li><li>500 fotografii + film</li><li>Album tipărit</li></ul>
            <a href="#contact" className="block mt-6 bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-full font-medium">Rezervă</a>
          </div>
          <div className="bg-gray-50 p-8 rounded-2xl reveal">
            ...
            <ul className="mt-4 text-left text-gray-500 space-y-2"><li>3 fotografi + 2 videografi</li><li>1000+ fotografii + film</li><li>Design personalizat</li></ul>
            <a href="#contact" className="block mt-6 bg-blue-500 hover:bg-blue-600 text-white py-2 rounded-full font-medium">Rezervă</a>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-gray-50 py-8 text-center text-gray-500">
        ...
        <p className="mt-2"><a href="#" className="text-blue-500 hover:underline">Instagram</a> | <a href="#" className="text-blue-500 hover:underline">Facebook</a> | <a href="tel:+37360000000" className="text-blue-500 hover:underline">+373 60 000 000</a></p>
      </footer>
    </main>
  );
}